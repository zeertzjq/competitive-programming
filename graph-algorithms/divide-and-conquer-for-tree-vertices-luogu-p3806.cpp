...
int n, m, e0[N], e1[E], c[E], dst[E], sz[N], tot, rt,
    rtmsz = N, dcnt = 0, vdcnt = 0, k[M], dist[N], vdist[2][N];
bool ans[M], vis[N], f;
...
inline void gans(int d) {
    vdist[f][vdcnt + 1] = 0;  // IMPORTANT: std::lower_bound may return a
                              // pointer to the element past the last
    for (int i = 1; i <= m; ++i)
        ans[i] |= *lower_bound(vdist[f] + 1, vdist[f] + 1 + vdcnt, k[i] - d) ==
                  k[i] - d;
}
...
void solve(int u) {
    ...
    for (int e = e0[u]; e; e = e1[e]) {
        ...
        merge(vdist[f] + 1, vdist[f] + 1 + vdcnt, dist + 1, dist + 1 + dcnt,
              vdist[!f] + 1);
        f ^= 1;
        vdcnt += dcnt;
    }
