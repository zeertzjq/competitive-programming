...
inline void init() {
    bs = sqrt(n), bc = n / bs;
    if (bs * bc < n) ++bc;
    for (int i = 1; i <= n; ++i) bi[i] = (i - 1) / bs + 1;
    for (int i = 1; i <= bc; ++i) ll[i] = (i - 1) * bs + 1, rr[i] = i * bs;
    rr[bc] = n;
    for (int i = 1; i <= bc; ++i) reset(i);
}

inline void upd(int l, int r, int w) {
    for (int i = l; i <= min(rr[bi[l]], r); ++i) a[i] += w;
    reset(bi[l]);
    if (bi[l] == bi[r]) return;
    for (int i = ll[bi[r]]; i <= r; ++i) a[i] += w;
    reset(bi[r]);
    for (int i = bi[l] + 1; i < bi[r]; ++i) t[i] += w;
}
