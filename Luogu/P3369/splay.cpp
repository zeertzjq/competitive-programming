...
int main() {
    ...
    while (_--) {
        ...
        if (opt == 1)
            ...
            rt = splay(rt, x);
            int res = rt;
            if (key[res] >= x) res = c[rt][0] = splay(c[rt][0], x);
            putln(key[res]);
        } else if (opt == 6) {
            rt = splay(rt, x);
            int res = rt;
            if (key[res] <= x) res = c[rt][1] = splay(c[rt][1], x);
            putln(key[res]);
        }
    }
}
