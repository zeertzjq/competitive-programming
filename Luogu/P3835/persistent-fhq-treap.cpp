...
inline int cp(int o) {
  key[++tot] = key[o], pri[tot] = pri[o], sz[tot] = sz[o], c[tot][0] = c[o][0],
  c[tot][1] = c[o][1];
  return tot;
}
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    if (o == 1) {
      int t1, t2;
      splt(rt[v], x, t1, t2), rt[i] = mrg(mrg(t1, mk(x)), t2);
    } else if (o == 2) {
      int t1, t2, t3;
      splt(rt[v], x, t1, t3), splt(t1, x - 1, t1, t2);
      if (t2)
        rt[i] = mrg(mrg(t1, mrg(c[t2][0], c[t2][1])), t3);
      else
        rt[i] = rt[v];
    } else if (o == 3) {
      int t1, t2;
      splt(rt[v], x - 1, t1, t2), putln(sz[t1] + 1), rt[i] = rt[v];
    } else if (o == 4) {
      putln(kth(rt[v], x)), rt[i] = rt[v];
    } else if (o == 5) {
      int t1, t2;
      splt(rt[v], x - 1, t1, t2), putln(t1 ? maxn(t1) : -inf), rt[i] = rt[v];
    } else {
      int t1, t2;
      splt(rt[v], x, t1, t2), putln(t2 ? minn(t2) : inf), rt[i] = rt[v];
    }
  }
  return 0;
}
