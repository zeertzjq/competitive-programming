...
inline void push(int x, int l, int r) {
  ...
  addt[R] = (addt[R] * mult[x] + addt[x]) % p, mult[L] = mult[L] * mult[x] % p,
  mult[R] = mult[R] * mult[x] % p, addt[x] = 0, mult[x] = 1;
}
...
void add(int x, int l, int r, int ll, int rr, long long v) {
  if (l > rr || r < ll)
    return;
  if (l >= ll && r <= rr) {
    sgt[x] = (sgt[x] + v * (r - l + 1)) % p, addt[x] = (addt[x] + v) % p;
    return;
  }
  push(x, l, r);
  int m = (l + r) >> 1;
...
void mul(int x, int l, int r, int ll, int rr, long long v) {
  if (l > rr || r < ll)
    return;
  if (l >= ll && r <= rr) {
    sgt[x] = (sgt[x] * v) % p, addt[x] = (addt[x] * v) % p,
    mult[x] = (mult[x] * v) % p;
    return;
  }
  push(x, l, r);
  int m = (l + r) >> 1;
  mul(L, l, m, ll, rr, v), mul(R, m + 1, r, ll, rr, v),
      sgt[x] = (sgt[L] + sgt[R]) % p;
}
