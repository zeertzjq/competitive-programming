...
void cdq1(int l, int r) {
    ...
    while (p2 <= r) f[q[p2].p] += qry(q[p2].v), tmp[p++] = q[p2++];
    for (int i = l; i <= r; ++i) clr(tmp[i].v), q[i] = tmp[i];
}
...
void cdq2(int l, int r) {
    ...
    while (p1 <= m) tmp[p++] = q[p1++];
    while (p2 <= r) f[q[p2].p] += qry(q[p2].p), tmp[p++] = q[p2++];
    for (int i = l; i <= r; ++i) clr(tmp[i].p), q[i] = tmp[i];
}
