...
inline void upd(int p, int x, int w) {
    for (; p <= n; p += p & -p) update(bit[p], 1, v[0], x, w);
}

inline void qinit(int *q, int p) {
    for (q[0] = 0; p; p &= p - 1) q[++q[0]] = bit[p];
}

inline void down(bool d) {
    for (int i = 1; i <= ql[0]; ++i) ql[i] = c[ql[i]][d];
    for (int i = 1; i <= qr[0]; ++i) qr[i] = c[qr[i]][d];
}

inline int lsz() {
    int ans = 0;
    for (int i = 1; i <= qr[0]; ++i) ans += s[c[qr[i]][0]];
    for (int i = 1; i <= ql[0]; ++i) ans -= s[c[ql[i]][0]];
    return ans;
}

inline int sz() {
    int ans = 0;
    for (int i = 1; i <= qr[0]; ++i) ans += s[qr[i]];
    for (int i = 1; i <= ql[0]; ++i) ans -= s[ql[i]];
    return ans;
}
