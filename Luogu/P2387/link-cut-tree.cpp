...
inline void upd(int x) {
    mx[x] = x;
    if (b[mx[c[x][0]]] > b[mx[x]]) mx[x] = mx[c[x][0]];
    if (b[mx[c[x][1]]] > b[mx[x]]) mx[x] = mx[c[x][1]];
}
...
int main() {
    ...
    sort(e + 1, e + 1 + m);
    for (int i = 1; i <= n + m; ++i) uf[i] = mx[i] = i;
    for (int i = 1; i <= m; ++i) b[n + i] = e[i].b;
    for (int i = 1; i <= m; ++i) {
        ...
        if (ur == vr) {
            ...
            uf[vr] = ur;
        if (f) link(u, i + n), link(i + n, v);
        if (finds(1) == finds(n)) ans = min(ans, e[i].a + b[pmx(1, n)]);
    }
    putln(ans == inf ? -1 : ans);
    return 0;
}
